...
export function isBrandNewDayListed(result: MonitorResult) {
  const pattern = new RegExp(config.ALERT_MOVIE_PATTERN, "i");
  return Boolean(result.movieNames?.some((name) => pattern.test(name)) || result.showtimes.some((show) => pattern.test(show.movieName || "")));
}
...
export async function sendCinemaSummaryEmail(result: MonitorResult) {
  ...
  const subject = bndOpen ? "Spider-Man: Brand New Day tickets are OPEN" : bndListed ? "Spider-Man: Brand New Day is LISTED - tickets not open" : `Allu Cinemas - ${config.SUMMARY_EMAIL_INTERVAL_MINUTES} minute availability summary`;
