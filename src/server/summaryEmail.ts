...
export function isBrandNewDayOpen(result: MonitorResult) {
  ...
  const movieMatches = pattern.test(result.movieName || "") || result.movieNames?.some((name) => pattern.test(name)) || result.showtimes.some((show) => pattern.test(show.movieName || ""));
  return movieMatches && result.showtimes.some((show) => show.enabled);
}
...
export async function sendCinemaSummaryEmail(result: MonitorResult) {
  ...
  const subject = bndOpen ? "🚨 Spider-Man: Brand New Day tickets are OPEN" : "Allu Cinemas — 10 minute availability summary";
